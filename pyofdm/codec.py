"""
OFDM transmitter and receiver with energy dispersal, pilot tones and cyclic
prefix.
"""
...
#                 2025 Yassin Achengli <achengli@github.com>
...
def setpilotindex(nData, mQAM, pilotspacing):
    """
    Returns an array of evenly spaced pilots which can then be used for
    pilotIndices in the OFDM constructor.
    Paramaters nData and mQAM must match the OFDM constructor arguments.
    """
    kpilot = (4 * nData // mQAM + pilotspacing // 2) // (pilotspacing - 1)
    pilotindex = np.arange(pilotspacing // 2,
                           kpilot * pilotspacing+pilotspacing // 2,
                           pilotspacing, dtype=int)
...
class OFDM:
    """
    OFDM encoder and decoder. The data is encoded as QAM using
    the komm package.

    Energy dispersal is done with a pre-seeded random number generator.
    Both pilot tones and the cyclic prefix are added so that the start
    of the symbol can be detected at the receiver.

    The complex time series after the inverse Fourier Transform can be
    modulated into a real valued stream with a Nyquist quadrature
    modulator for baseband.

    On the receiver side the start of the symbol is detected by first
    doing a coarse search with the cyclic prefix and then a precision
    alignment with the pilots.
    """

    def __init__(self, nFreqSamples=64, pilotIndices=[-21, -7, 7, 21],
                 pilotAmplitude=1, nData=12, fracCyclic=0.25, mQAM=2):
        """
        nFreqSamples sets the number of frequency coefficients of the FFT.
        Pilot tones are injected at pilotIndices. The real valued pilot
        amplitude is pilotAmplitude.
        For transmission nData bytes are expected in an array. The relative
        length of the Cyclic prefix is fracCyclic.
        Number of QAM symbols = 2**mQAM, giving mQAM bits per QAM symbol.
        Average power is normalised to unity. Default example correspond
        to 802.11a wifi modulation.
        """
    ...
    def encode(self, data, randomSeed=1):
        ...
        # create an empty spectrum with all complex frequency values set to
        # zero
        ...
        for k in range(-self.kstart, 0):
            if ipilot == self.pilotIndices.size or \
                    k != self.pilotIndices[ipilot]:
    ...
    def decode(self, randomSeed=1):
        ...
        # Perform an FFT to get the frequency samples which code our signal
        # as QPSK pairs
        ...
        # set the random number generator to the same value as in the
        # transmitter so that we have exactly the same sequence
        rng = np.random.default_rng(randomSeed)
    ...
    def findSymbolStartIndex(self, signal, searchrangecoarse=None,
                             searchrangefine=25):
        """
        ...
        pks, _ = scipy.signal.find_peaks(crosscorr, distance=self.nIFFT,
                                         width=self.nCyclic//2)
        o1 = pks[0]
    ...
    def EVM(self):
        ...
        constellation = OFDM.genQAMConstellation(self.mQAM)
        evm = np.zeros(self.signal.shape)

        for idx, sig in enumerate(self.signal):
            distance = constellation - sig
            min_distance = np.min(distance)
            evm[idx] = min_distance**2

        return evm

    def meanEVM(self):
        return np.mean(self.EVM())
